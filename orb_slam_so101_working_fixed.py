#!/usr/bin/env python3
"""
Working ORB-SLAM SO101 Teleoperator with Fixed Joint State

This version fixes the initial joint state issue and uses a more robust approach.
"""
...
class WorkingFixedOrbSlamSo101Teleoperator:
    """Working ORB-SLAM SO101 teleoperator with fixed joint state."""
    ...
    def __init__(self):
        ...
        self.so101_config = SO101FollowerConfig(
            port="/dev/ttyACM0",
            use_degrees=True,
            max_relative_target=10.0,  # Smaller for smoother movement
        ...
        # SO101 Workspace Limits (in meters)
        self.robot_workspace_limits = {
            'x': [-0.2, 0.2],    # 40cm horizontal range
            'y': [-0.2, 0.2],    # 40cm depth range
            'z': [0.15, 0.35]    # 20cm vertical range
        }
        
        # Scaling factors
        self.scale_factor = 0.02  # 2% of camera movement
        ...
        if not self.is_running:
            ...
            try:
                while self.is_running:
                    ...
                    time.sleep(0.2)  # 5Hz for smoother control
    ...
    def _solve_ik_and_control(self, target_pose: np.ndarray):
        ...
        try:
            ...
            if joint_angles is not None:
                ...
                if 0.005 < max_diff < 0.3:  # Between 0.3° and 17°
...
def main():
    """Main function."""
    print("🎯 Working Fixed ORB-SLAM SO101 Teleoperation")
    print("=" * 50)
    
    # Create teleoperator
    teleoperator = WorkingFixedOrbSlamSo101Teleoperator()
