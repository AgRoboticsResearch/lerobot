...
def create_robot(can_name, home_deg, speed):
    """Connect, enable, move to home, read T_base."""
...
def load_trajectory(csv_path, T_base, mode, max_steps=None):
    ...
    for _, row in df.iterrows():
        if mode == "ee":
            ...
            joints = np.array([row[f"action.joint{i+1}"] for i in range(NUM_ARM_JOINTS)])
            gripper = row["action.gripper_pos"]
            traj.append({"joints": joints.copy(), "gripper": gripper})

    if max_steps:
        traj = traj[:max_steps]
    print(f"Prepared {len(traj)} trajectory steps ({mode} mode)")
    return traj
...
def exec_step(piper, step, i, speed, mode):
    ...
    if mode == "ee":
        ...
        if was_clamped:
            print(f"  WARNING: joints clamped {np.round(q, 2)} → {np.round(q_clamped, 2)}")
...
def read_actual(piper, step, mode):
    """Read actual state from robot for logging."""
    if mode == "ee":
        pos_m, euler_deg = read_ee_pose(piper)
        ...
        q_target = step["joints"]
        joint_err = np.max(np.abs(q_target - q_actual))
        return q_target, q_actual, joint_err
...
def run_trajectory(piper, traj, step_time, speed, mode):
    ...
    if mode == "ee":
        ...
        print(f"First joints: {np.round(traj[0]['joints'], 2)}°")
        print(f"Last joints:  {np.round(traj[-1]['joints'], 2)}°")
    print(f"{'='*60}")
    ...
    try:
        for i, step in enumerate(traj):
            exec_step(piper, step, i, speed, mode)
            time.sleep(step_time)

            # Read actual state
            if mode == "ee":
                T_sent, T_actual, pos_err = read_actual(piper, step, mode)
                ...
                q_sent, q_actual, joint_err = read_actual(piper, step, mode)
                sent_p.append(q_sent.copy())
                act_p.append(q_actual.copy())
                print(f"  Step {i:3d}: max_joint_err={joint_err:.2f}°")
                print(f"    Sent: {np.round(q_sent, 2)}")
                print(f"    True: {np.round(q_actual, 2)}")
                errors.append(joint_err)
    ...
    if errors:
        ...
        print(f"  Mean error: {np.mean(errors):.3f}{'mm' if mode == 'ee' else '°'}")
        print(f"  Max  error: {np.max(errors):.3f}{'mm' if mode == 'ee' else '°'}")

    result = dict(
        sent_pos=np.array(sent_p), act_pos=np.array(act_p),
        sent_rv=np.array(sent_r) if mode == "ee" else np.zeros((len(sent_p), 3)),
        act_rv=np.array(act_r) if mode == "ee" else np.zeros((len(act_p), 3)),
...
def plot(result, out_dir, mode):
    ...
    if mode == "ee":
        ...
        sd = np.column_stack([sp, gs.reshape(-1, 1)])
        ad = np.column_stack([ap, ga.reshape(-1, 1)])

    # 3D plot (EE mode only)
    if mode == "ee":
        ...
        ax.set_title("3D EE Trajectory [Piper Real]"); ax.legend()
        plt.tight_layout(); fig.savefig(str(out_dir / "traj_3d.png")); plt.close()

    # 2D plots
    n_plots = sd.shape[1]
    fig, axes = plt.subplots(n_plots, 1, figsize=(12, n_plots * 2.5), sharex=True)
    ...
    axes[0].set_title(f"State [Piper Real, {mode} mode]")
...
def main():
    ...
    # Connect and home
    piper, T_base = create_robot(args.can_name, home_deg, args.speed)

    # SIGINT handler: disable on Ctrl+C
    def on_sigint(sig, frame):
        print("\nSIGINT: disabling robot...")
        try:
            piper.DisablePiper()
        except Exception:
            pass
    ...
    traj = load_trajectory(args.traj_csv, T_base, args.mode, args.steps)
    result = run_trajectory(piper, traj, args.step_time, args.speed, args.mode)
