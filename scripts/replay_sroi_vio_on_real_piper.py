...
# Assuming lerobot_robot_piper is in the python path or installed
# If not, we might need to adjust sys.path, but usually it should be installed in the environment.
try:
    from lerobot_robot_piper.piper import Piper
    from lerobot_robot_piper.config_piper import PiperConfig
except ImportError:
    # Fallback if package is not installed but folder exists in workspace
    import sys
...
"""
Replays the relative VIO trajectory on a REAL Piper robot and computes the error.
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Replay VIO trajectory on REAL Piper robot and compute error")
    parser.add_argument("file_path", help="Path to the trajectory file")
    parser.add_argument("--can_interface", default="can0", help="CAN interface for the robot (e.g., can0)")
    parser.add_argument("--output", default="piper_error_log.csv", help="Output file for error log")
    ...
    print(f"Initializing Piper on {args.can_interface}...")
    robot_config = PiperConfig(
        can_interface=args.can_interface,
        use_degrees=True,
        include_gripper=True,
        cameras={}, # Disable cameras for this script
        joint_names=[f"joint{i+1}" for i in range(6)] # Override to match URDF and script expectations
    )
    real_robot = Piper(robot_config)
    ...
    # Assuming piper_description is in the workspace root/piper_description
    urdf_path = os.path.join(current_dir, "../piper_description/urdf/piper_description.urdf")
    effector_frame = "gripper_base" # As used in replay_sroi_vio_on_robot_lerobotIK.py
