...
- `per_episode/` — 93 compact per-run files (`<RUN>.json.gz`): episode-
  ...
  + 5 Q3 two-frame rows added 2026-08-25, §9.2.17.)
- `physical_dynamics_h10.{csv,md}` — repository-tracked 97-row summary
  (88 §9.2.9 torch rows + 5 Q3 two-frame rows + four physical-pending JAX
  rows), with matched point
