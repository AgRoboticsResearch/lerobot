...
- `per_episode/` — 103 compact per-run files (`<RUN>.json.gz`): episode-
  ...
  2026-08-26, §9.2.18.)
- `physical_dynamics_h10.{csv,md}` — repository-tracked 103-row summary
  (88 §9.2.9 torch rows + 5 Q3 two-frame rows + 5 JAX openpi rows + 5 Q4
  no-proprio rows, all with measured physical metrics), with matched point
...
- `configs/` — per-run training configuration (`<RUN>.config.json`) copied
  ...
  (5 each, added 2026-08-25/26); `per_episode/*.json.gz` record the exact
