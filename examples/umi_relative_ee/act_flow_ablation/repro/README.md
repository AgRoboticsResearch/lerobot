...
- `per_episode/` — 121 compact per-run files (`<RUN>.json.gz`): episode-
...
- `physical_dynamics_h10.{csv,md}` — repository-tracked 121-row summary
  (88 §9.2.9 torch rows + 5 Q3 two-frame rows + 5 JAX openpi rows + 5 Q4
  no-proprio rows + 3 LingBot-VA rows + 15 Q4 state-window rows
  (W=3/5/10), all with measured physical metrics), with matched point
...
- `configs/` — per-run training configuration (`<RUN>.config.json`) copied
  ...
  (5 each, added 2026-08-25/26), the Q4 state-window arms (W=3/W=5 host
  `train/act_r50_v1_vae_state{3,5}…` / W=10 kiwi
  `/mnt/data/zfei/lerobot-act-flow-ablation/train/act_r50_v1_vae_state10…`;
  5 each, added 2026-08-29, §9.2.20), and the LingBot-VA arm (3, added
