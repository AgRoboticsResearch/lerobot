...
- `per_episode/` — 126 compact per-run files (`<RUN>.json.gz`): episode-
  ...
  protocol block. Written by `../compile_physical_jerk.py`. (88 §9.2.9 rows
  + 5 Q3 two-frame rows added 2026-08-25, §9.2.17 + 5 JAX openpi rows
  re-scored 2026-08-25, §9.2.13/§9.2.14 + 5 Q4 no-proprio rows added
  2026-08-26, §9.2.18 + 3 LingBot-VA rows added 2026-08-26, §9.2.16
  + 10 Q4 state-window rows (W=5 host / W=10 kiwi) added 2026-08-29,
  §9.2.20 + 5 Q4 state-window W=3 rows (host) added 2026-08-29, §9.2.20
  + 5 Q4 state-window W=4 rows (host) added 2026-08-30, §9.2.20.)
- `physical_dynamics_h10.{csv,md}` — repository-tracked 126-row summary
  (88 §9.2.9 torch rows + 5 Q3 two-frame rows + 5 JAX openpi rows + 5 Q4
  no-proprio rows + 3 LingBot-VA rows + 20 Q4 state-window rows
  (W=3/4/5/10), all with measured physical metrics),
  with matched point estimate / 95% CI columns for rotational and XYZ
  velocity, acceleration, and jerk. Regenerated by the canonical compiler
  invocation below.
- `cross_frame_mse.{csv,md}` — all 134 checkpoints / 262 native-horizon
  rows from the adjacent-frame direct prediction-change sweep (§9.2.19):
  normalized action, XYZ, rotvec, SO(3) geodesic, and gripper MSE for
  independent t/t+1 calls at h10/h30.
- `per_episode_cross_frame_mse/` — 134 compact gzip JSON files with protocol,
  normalization, point estimates, 95% CIs, and per-episode values for that
  sweep. The large sample-level reports remain in the external eval root.
...
- `configs/` — per-run training configuration (`<RUN>.config.json`) copied
  verbatim from each archived checkpoint (draccus-saved). The 88 §9.2.9 names
  map 1:1 to that tree's runs, plus the Q3 two-frame and Q4 no-proprio arms
  (5 each, added 2026-08-25/26), the Q4 state-window arms (W=3/W=4/W=5 host
  `train/act_r50_v1_vae_state{3,4,5}…` / W=10 kiwi
  `/mnt/data/zfei/lerobot-act-flow-ablation/train/act_r50_v1_vae_state10…`;
  5 each, added 2026-08-29, §9.2.20), and the LingBot-VA arm (3, added 2026-08-26;
  trained on-host at
  `/mnt/data1/projects/lerobot-arch-exp/train/lingbot_va_axis_angle_seed1000_200000steps/`
  — its PEFT checkpoints carry the policy `config.json` + LoRA
  `adapter_config.json`, not a draccus training dump);
  `per_episode/*.json.gz` record the exact checkpoint each row was scored
  against.
...
# 5. §9.2.19 adjacent-frame direct MSE (after its kiwi/OpenPI sweeps):
uv run python examples/umi_relative_ee/act_flow_ablation/compile_cross_frame_mse.py \
  --eval_root /mnt/data1/projects/lerobot-arch-exp/reeval_v2metrics/eval_cross_frame_mse/tree \
  --manifest /mnt/data1/projects/lerobot-arch-exp/reeval_v2metrics/eval_cross_frame_mse/manifest.tsv \
  --openpi_root /mnt/data1/projects/lerobot-arch-exp/reeval_v2metrics/eval_cross_frame_mse/openpi_tree \
  --out_dir /tmp/cross_frame_mse_results \
  --per_episode_dir examples/umi_relative_ee/act_flow_ablation/repro/per_episode_cross_frame_mse
MPLCONFIGDIR=/tmp/lerobot-matplotlib /home/zfei/anaconda3/envs/py312/bin/python \
...
- Eval trees: `/mnt/data1/projects/lerobot-arch-exp/reeval_v2metrics/`
  ...
  CSVs incl. `results_salvage_h10/`, `eval_lingbot_h10/` §9.2.16 (3-point
  LingBot-VA budget curve; rows also mirrored into the two archive trees
  above), and `eval_cross_frame_mse/` with the
  complete §9.2.19 raw tree, manifest, logs, statuses, and compiled results).
- Checkpoint archive (only copy): kiwi
  ...
  + `state10_h10/` — the §9.2.20 kiwi-side W=10 canonical evals, rsynced
  back and mirrored into the two host archive trees),
