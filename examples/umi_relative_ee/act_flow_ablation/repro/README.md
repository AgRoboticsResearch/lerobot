...
- `per_episode/` — 113 compact per-run files (`<RUN>.json.gz`): episode-
  ...
  2026-08-26, §9.2.18 + 10 Q4 state-window rows (W=5 host / W=10 kiwi)
  added 2026-08-29, §9.2.20.)
- `physical_dynamics_h10.{csv,md}` — repository-tracked 113-row summary
  (88 §9.2.9 torch rows + 5 Q3 two-frame rows + 5 JAX openpi rows + 5 Q4
  no-proprio rows + 10 Q4 state-window rows, all with measured physical
  metrics), with matched point
...
- `configs/` — per-run training configuration (`<RUN>.config.json`) copied
  ...
  (5 each, added 2026-08-25/26) and the Q4 state-window arms (W=5 host
  `train/act_r50_v1_vae_state5…` / W=10 kiwi
  `/mnt/data/zfei/lerobot-act-flow-ablation/train/act_r50_v1_vae_state10…`;
  5 each, added 2026-08-29, §9.2.20); `per_episode/*.json.gz` record the
  exact checkpoint each row was scored against.
