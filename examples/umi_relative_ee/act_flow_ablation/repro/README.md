...
- `per_episode/` — 98 compact per-run files (`<RUN>.json.gz`): episode-
  ...
  re-scored 2026-08-25, §9.2.13/§9.2.14.)
- `physical_dynamics_h10.{csv,md}` — repository-tracked 98-row summary
  (88 §9.2.9 torch rows + 5 Q3 two-frame rows + 5 JAX openpi rows, all
  with measured physical metrics), with matched point
