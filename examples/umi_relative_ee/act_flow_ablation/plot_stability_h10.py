...
kiwi stability tree) and renders:
  ...
  figures/stability_h10_scores.png — re-query disagreement at k=1 and k=10
    (XYZ mm + rotation deg, episode-balanced, 95% bootstrap CIs), all 16
...
FAMILY_PREFIXES = [
    ("act_umi_identity_rot6d_1459_", "hist", "R18-VAE hist 3M", "#444444"),
    ("act_r50_v1_vae_2frame_", "2frame", "R50-V1 2-frame 500k (Q3)", "#8c564b"),
    ("act_r50_v1_vae_", "r50v1", "R50-V1", "#1f77b4"),
    ("act_r18_l1_", "actl1", "ACT-L1", "#2ca02c"),
    ("act_r50_vae_", "r50vae", "R50-VAE", "#9467bd"),
    ...
    ("pi05_port_openpi_recipe_", "port20k", "π0.5 port o-recipe 20k", "#7f7f7f"),
...
def short(run: str) -> str:
    ...
    steps = f"{int(steps) // 1000}k" if int(steps) >= 1000 else steps
...
def fig_scores(rows: dict) -> None:
    ...
    for ax, k, title in zip(axes, (1, 10), ("re-query interval k=1 (async-replan regime)", "k=10")):
    ...
    axes[0].set_yticks(list(range(len(labels))), labels=[f"{l}  " for l in labels], fontsize=8)
    axes[0].invert_yaxis()
...
def fig_growth(rows: dict) -> None:
    ...
    for ax, title in zip(axes, ("XYZ disagreement (mm)", "Rotation disagreement (deg)")):
        ax.set_xticks(ks)
