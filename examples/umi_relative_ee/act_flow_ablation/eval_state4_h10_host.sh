#!/usr/bin/env bash
# Canonical §9.2.9 h10 eval sweep for the Q4 state-window W=4 arm (host).
...
# shift with the state window (W=4 -> 3 leading negative deltas).
set -uo pipefail
...
for STEP in 100000 200000 300000 400000 500000; do
  ...
  RUN_DIR="$ROOT/reeval_v2metrics/eval_unified_h10/act_r50_v1_vae_state4_seed1000_$(printf '%07d' "$STEP")steps"
  ...
  JERK="$ROOT/reeval_v2metrics/eval_unified_h10_jerk/act_r50_v1_vae_state4_seed1000_$(printf '%07d' "$STEP")steps/seed1000"
  mkdir -p "$JERK"
  cp "$OUT/${TRAIN_RUN}_${STEP}_open_loop_metrics.json" "$JERK"/
done
echo "[$(date '+%F %T')] sweep complete: $TRAIN_RUN"
