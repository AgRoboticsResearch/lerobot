...
camera image. This doc covers the shared method; for exact run commands see
`visualize_predictions_pi05.md` (π0.5, this repo) and
`visualize_predictions_act_smolvla.md` (ACT/SmolVLA, sibling `lerobot` repo).
...
> (The sibling script was originally URDF/placo-based and was switched to the JSON
> via `--extrinsics_config`.)

## The two scripts
| script (repo / env) | models | output | notes |
|---------------------|--------|--------|-------|
| `lerobot-fei-v5.0/examples/umi_relative_ee/visualize_predictions.py` (py312) | π0.5 LoRA | `pred_episode_<N>.mp4` | panel composite (camera + 3D + per-dim curves); on-image projection with `--project` |
| `lerobot/examples/umi_relative_ee/visualize_predictions.py` (py310) | ACT, SmolVLA | `proj_inference_episode_<N>.mp4` + `traj3d_…mp4` | on-image projection + separate 3D video |

Both copy sroi's projection helpers (`load_tip_kin`, `project_future`,
...
Swap `--dataset_root` (and `--camera_info_path` for the sibling script). Train
...
- **Hand-eye**: `camera_gripper_extrinsics_sroi_v2_d405.json` next to each script.
- **Envs**: py312 (π0.5 / fei-v5.0), py310 (ACT / SmolVLA / sibling).
...
π0.5 on validation (this repo, py312):
```bash
...
```
SmolVLA on validation (sibling, py310):
```bash
cd /mnt/data0/code/lerobots/lerobot
...
```
(ACT is the same as SmolVLA but without `--task`, and the π0.5 checkpoint path.)
