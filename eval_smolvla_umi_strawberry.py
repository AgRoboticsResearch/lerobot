#!/usr/bin/env python
"""Eval SmolVLA UMI EE-pose on sroi_piper_strawberry_picking.
...
Verbose mode (--verbose) traces one batch through the full pipeline:
  raw → derive_state → relative → relative_state → normalize → model predict → unnormalize → absolute
"""
...
DATASET_REPO = "sroi_piper_strawberry_picking"
DEFAULT_CHECKPOINT = "/home/hls/codes/lerobot_piper_sroi/outputs/smolvla_umi_strawberry_50k/checkpoints/050000/pretrained_model"
...
def eval_checkpoint(checkpoint_dir: str, n_batches: int = 50, verbose: bool = False, max_steps: int = 50):
    ...
    for j, step in enumerate(preprocessor.steps):
        ...
        if isinstance(step, DeriveStateFromActionStep):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(dataloader, total=n_batches)):
            ...
            gt_state = batch["observation.state"]
            ...
            if i == 0 and verbose:
                ...
                for t in range(min(3, gt_action.shape[1])):
                    print(f"    t={t}: {gt_action[0, t].cpu().tolist()}")

            processed = preprocessor(batch)

            if i == 0 and verbose:
                print(f"\n--- STEP 1: After preprocess (derive_state → relative → relative_state → normalize) ---")
                _print_tensor("processed['action']", processed["action"])
                _print_tensor("processed['observation.state']", processed["observation.state"])

            pred_action = policy.predict_action_chunk(processed)
            ...
            if i == 0 and verbose:
                ...
                for t in range(min(3, pred_action.shape[1])):
            ...
            if i == 0 and verbose:
                ...
                n_action_steps = gt_action.shape[1] - 1 if gt_action.shape[1] > 1 else gt_action.shape[1]
                print(f"\n--- STEP 4: Comparison (sample 0, first 5 timesteps) ---")
                print(f"  Note: DeriveStateFromAction strips leading step, so GT has {gt_action.shape[1]} steps, pred has {pred_abs.shape[1]} steps")
                print(f"  Comparing first {min(n_action_steps, pred_abs.shape[1])} steps")
                ...
                print(f"  {'':>3}  {'':>8} {'':>8} {'(mm)':>8}  "
                      f"{'':>8} {'':>8} {'(mm)':>8}  "
                      f"{'':>8} {'':>8}")
                compare_len = min(5, n_action_steps, pred_abs.shape[1])
                for t in range(compare_len):
                    gt_x = gt_action[0, t + 1, 0].item()
                    pred_x = pred_abs[0, t, 0].item()
                    gt_y = gt_action[0, t + 1, 1].item()
                    pred_y = pred_abs[0, t, 1].item()
                    gt_g = gt_action[0, t + 1, 6].item()
                ...
                min_len_v = min(n_action_steps, pred_abs.shape[1])
                err = (pred_abs[0, :min_len_v].cpu() - gt_action[0, 1:1 + min_len_v].cpu()).abs()
                print(f"\n  Error across chunk, sample 0:")
            ...
            n_action_steps = gt_action.shape[1] - 1 if gt_action.shape[1] > 1 else gt_action.shape[1]
            min_len = min(n_action_steps, pred_abs.shape[1], max_steps)
            gt = gt_action[:, 1:1 + min_len, :n_action_dims].cpu()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Eval SmolVLA UMI EE-pose on strawberry picking dataset")
    parser.add_argument("checkpoint_dir", nargs="?", default=DEFAULT_CHECKPOINT,
                        help="Path to checkpoint pretrained_model dir")
