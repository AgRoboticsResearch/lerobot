...
class LingBotVAPolicy(PreTrainedPolicy):
    ...
    def _get_default_peft_targets(self) -> dict:
        """Adapt every transformer linear layer while keeping the 5B base frozen.

        This includes the action input/output projections as well as attention,
        feed-forward, video-patch, and conditioning projections.  Restricting LoRA
        to attention alone would leave the task-specific action interface much less
        adaptable for the one-camera UMI domain shift.
        """
