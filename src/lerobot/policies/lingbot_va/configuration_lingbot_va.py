...
class LingBotVAConfig(PreTrainedConfig):
    ...
    # The strawberry dataset stores absolute 7D xyz/axis-angle/gripper poses.
    # The processor maps these to chunk-start-relative 7D actions before LingBot
    # scatters them into channels 0..6 of its fixed 30D latent action space.
    use_umi_relative_ee: bool = False
    # The derived 20D state remains rot6d even when LingBot's action is 7D
    # axis-angle. Identity state-rotation statistics preserve the shared UMI
    # geometry and make this candidate comparable to the other policies.
    ...
    def __post_init__(self):
        ...
        if len(set(self.used_action_channel_ids)) != len(self.used_action_channel_ids):
            raise ValueError("used_action_channel_ids must not contain duplicates")
        if any(channel < 0 or channel >= self.action_dim for channel in self.used_action_channel_ids):
            raise ValueError(
                f"used_action_channel_ids must be within [0, {self.action_dim}); "
                f"got {self.used_action_channel_ids}"
            )
        if self.use_umi_relative_ee and self.used_action_channel_ids != list(range(7)):
            raise ValueError(
                "UMI LingBot training requires channels 0..6 in xyz/axis-angle/gripper order; "
                f"got {self.used_action_channel_ids}"
            )
