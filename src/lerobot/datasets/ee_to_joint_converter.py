...
def convert_episode(
    ...
    for frame_idx in range(start_idx, end_idx + 1):
        ...
        aligned_ee_pose_7d = np.concatenate(
            [aligned_pos, aligned_rotvec, [ee_pose[6]]]
        )  # Keep original gripper
    ...
    successful_pos_errors = [pe for pe, s in zip(pos_errors, ik_successes, strict=True) if s]
    successful_rot_errors = [re for re, s in zip(rot_errors, ik_successes, strict=True) if s]
...
class EEToJointDatasetConverter:
    ...
    def convert_all(
        ...
        if episode_indices is not None:
            ...
            if invalid_indices:
                raise ValueError(
                    f"Invalid episode indices: {invalid_indices}. Valid range: 0-{total_episodes - 1}"
                )
        ...
        for ep_idx in episodes_to_convert:
            ...
            for frame_idx in range(start_idx, end_idx + 1):
                ...
                if frames_in_current_file >= chunks_size:
                    ...
                    write_parquet_with_fixed_size_arrays(
                        chunk_data, chunk_dir / f"file-{current_file:03d}.parquet"
                    )
                    chunk_data = []
                    frames_in_current_file = 0
                    current_file += 1
        ...
        for i in range(len(filtered_episodes)):
            ...
            new_ep = {
                ...
                "videos/observation.images.camera/from_timestamp": ep[
                    "videos/observation.images.camera/from_timestamp"
                ],
                "videos/observation.images.camera/to_timestamp": ep[
...
def plot_trajectory_comparison_3d(
    ...
    """
    num_frames = original_ee_poses.shape[0]
    ...
    ax1.plot(
        original_ee_poses[:, 0], original_ee_poses[:, 1], "b-", linewidth=2, label="Original EE", alpha=0.8
    )
    ax1.scatter(
        original_ee_poses[0, 0],
        original_ee_poses[0, 1],
        c="blue",
        s=100,
        marker="o",
        edgecolors="black",
    ...
    ax1.scatter(
        original_ee_poses[-1, 0],
        original_ee_poses[-1, 1],
        c="blue",
        s=100,
        marker="s",
        edgecolors="black",
    ...
    ax1.plot(fk_ee_poses[:, 0], fk_ee_poses[:, 1], "r--", linewidth=2, label="FK from Joint", alpha=0.8)
    ax1.scatter(
        fk_ee_poses[0, 0],
        fk_ee_poses[0, 1],
        c="red",
    ...
    if original_gripper is not None:
        for i in range(0, num_frames, max(1, num_frames // 20)):  # Show ~20 points
            size = 50 + original_gripper[i] * 100  # Scale by gripper state
            ax1.scatter(original_ee_poses[i, 0], original_ee_poses[i, 1], c="blue", s=size, alpha=0.3)
            ax1.scatter(fk_ee_poses[i, 0], fk_ee_poses[i, 1], c="red", s=size, alpha=0.3)
...
def save_trajectory_debug_viz(
    ...
    for local_idx in local_indices:
        ...
        for joint_pos in joint_positions[local_idx]:
            ee_transform = kinematics.forward_kinematics(joint_pos.astype(np.float64))
            fk_ee_poses.append(ee_transform[:3, 3].copy())
