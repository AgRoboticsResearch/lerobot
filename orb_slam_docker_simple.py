#!/usr/bin/env python3
"""
Simple ORB-SLAM3 Docker Integration for SO101 Teleoperation

This version directly runs the ORB-SLAM3 Docker container with RealSense D435i.
"""
...
class SimpleOrbSlamDockerIntegration:
    """Simple integration with ORB-SLAM3 Docker container."""
    
    def __init__(self):
        """Initialize the simple ORB-SLAM3 Docker integration."""
        self.docker_image = "lmwafer/orb-slam-3-ready:1.1-ubuntu18.04"
        self.container_name = "orb_slam3_simple"
        self.process = None
        ...
        # Pose parsing
        self.pose_pattern = re.compile(r'pose:\s*\[([^\]]+)\]')
        
        print(f"✅ Simple ORB-SLAM3 Docker integration initialized")
        print(f"🐳 Using Docker image: {self.docker_image}")
    
    def start_orb_slam3(self):
        """Start ORB-SLAM3 with RealSense D435i."""
        try:
            print(f"🚀 Starting ORB-SLAM3 with RealSense D435i...")
            ...
            cmd = [
                ...
                self.docker_image,
                "/bin/bash", "-c",
                "cd /dpds/ORB_SLAM3 && ./Examples/Stereo/stereo_realsense_D435i Vocabulary/ORBvoc.txt Examples/Stereo/RealSense_D435i.yaml"
            ]
    ...
    def _parse_pose_from_output(self, line):
        ...
        try:
            ...
            # For now, let's simulate pose updates
            if "tracking" in line.lower() or "pose" in line.lower():
...
class SimpleOrbSlamSo101Teleoperator:
    """SO101 teleoperator using simple ORB-SLAM3 Docker integration."""
    ...
    def __init__(self):
        ...
        self.so101_config = SO101FollowerConfig(
            port="/dev/ttyACM0",
            use_degrees=True,
            max_relative_target=2.0,  # Very small for smooth movement
        ...
        self.robot_workspace_limits = {
            'x': [-0.03, 0.03],   # 6cm horizontal range
            'y': [-0.03, 0.03],   # 6cm depth range
            'z': [0.28, 0.32]     # 4cm vertical range
        }
        
        # Scaling factors - very small for safety
        self.scale_factor = 0.002  # 0.2% of camera movement
        ...
        print("✅ Simple ORB-SLAM3 Docker SO101 teleoperator initialized")
        print(f"📏 Robot workspace: X{self.robot_workspace_limits['x']}, Y{self.robot_workspace_limits['y']}, Z{self.robot_workspace_limits['z']}")
        print(f"🔧 Scale factor: {self.scale_factor} (0.2% of camera movement)")
    ...
    def connect(self):
        ...
        try:
            ...
            print("🐳 Starting ORB-SLAM3 Docker...")
            self.orb_slam_docker = SimpleOrbSlamDockerIntegration()
            if not self.orb_slam_docker.start_orb_slam3():
                print("❌ Failed to start ORB-SLAM3")
                return False
            print("✅ ORB-SLAM3 Docker started")
    ...
    def _solve_ik_and_control(self, target_pose: np.ndarray):
        ...
        try:
            ...
            if joint_angles is not None:
                ...
                if 0.0002 < max_diff < 0.02:  # Between 0.01° and 1.1°
...
def main():
    """Main function."""
    print("🎯 Simple ORB-SLAM3 Docker SO101 Teleoperation")
    print("=" * 50)
    
    # Create teleoperator
    teleoperator = SimpleOrbSlamSo101Teleoperator()
